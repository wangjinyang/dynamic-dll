...
export class BuildDepPlugin {
  ...
  constructor(opts: IOpts) {
    ...
    this.opts = opts;
    const { parseOptions } = require(join(
      this.opts.dynamicDll.opts.webpackPath,
      "webpack/lib/container/options"
    ));
    ...
    compiler.hooks.normalModuleFactory.tap(PLUGIN_NAME, (nmf) => {
      ...
      nmf.hooks.createModule.tap(PLUGIN_NAME, (_createData, resolveData) => {
        if (!NODE_MODULES.test(resolveData.context)) {
          ...
          let isMatch = false;
          if (
            this.opts.dynamicDll.opts.includesLibs.some((p) => p.test(request))
          ) {
            isMatch = true;
          }
          if (!isMatch) {
            isMatch =
              NODE_MODULES.test(resource as string) &&
              !this.opts.dynamicDll.opts.excludeLibs.some((p) =>
                p.test(request)
              );
          }
          if (isMatch) {
            if (!request.startsWith(".") && !request.includes("!")) {
              ...
              const {
                resourceResolveData: {
                  descriptionFileData: { version = null },
                } = {},
              } = createData;
              ...
              if (!dep) {
                ...
                const { libraryPath: oldLibraryPath, version: oldVersion } =
                  dep;
              ...
              for (const [key, config] of remotes) {
                if (
                  ...
                  const RemoteModule = require(join(
                    this.opts.dynamicDll.opts.webpackPath,
      ...
      if (!stats.hasErrors()) {
        if (this.needBuild) {
          ...
          this.timer = setTimeout(() => {
            this.opts.dynamicDll.buildDeps(
